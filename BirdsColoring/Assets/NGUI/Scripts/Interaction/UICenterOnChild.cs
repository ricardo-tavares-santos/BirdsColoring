...
public class UICenterOnChild : MonoBehaviour
{
	...
	public void Recenter ()
	{
		...
		if (grid != null)
		{
			...
			for (int i = 0, imax = list.Count, ii = 0; i < imax; ++i)
			{
				...
				if(!GameManager.Instance.isImageClicked)
					t.GetComponent<TweenScale>().PlayReverse();								// My Chaipiiiiiii
	...
	void CenterOn (Transform target, Vector3 panelCenter)
	{
		if (target != null && mScrollView != null && mScrollView.panel != null)
		{
			...
			if(!GameManager.Instance.isImageClicked)
			{
				target.GetComponent<TweenScale>().PlayForward();					//My Chaipiiii
				...
				for (int i = 0, imax = trans.childCount; i < imax; ++i)
				{
					...
					{
						t.GetComponent<TweenScale>().PlayReverse();								//my chaipiiii ends
					}
				}
			}
